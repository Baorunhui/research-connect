...
飞书不能由机器人向原生输入框工具栏注入自定义图标，自定义菜单也不支持持久的选中/勾选状态。当前实现使用机器人聊天页底部的单个“🌐 联网开关”菜单项：第一次点击从默认 `auto` 切换为 `on`，再次点击切换为 `off`，机器人每次回复当前状态。命令继续作为精确控制和兜底：

```text
/web auto   # 默认；模糊任务或时效问题才联网
/web on     # 后续消息始终先联网
/web off    # 禁止联网
/web        # 查看当前状态
```

也支持“自动联网”“开启联网”“关闭联网”。菜单点击事件只包含用户 `open_id`，不包含当前会话 ID，因此菜单设置会作为用户级默认值并同步到该用户的已知会话；会话内命令仍可单独覆盖。设置全部持久化到 SQLite。
...
10. 在“应用能力 → 机器人 → 机器人自定义菜单”添加一个菜单项：
    - 菜单名称：`🌐 联网开关`；
    - 类型：选择“推送事件”；
    - 事件键：`connect_web_toggle`。
11. 创建新版本、发布应用，并设置应用可用范围。企业租户可能需要管理员审批；已经发布过的应用在新增事件或菜单后也要再次发布。
12. 单聊搜索机器人，或者将机器人添加到群聊后 @它。菜单发布后会出现在机器人聊天页底部；重复点击同一菜单项即可开关联网，机器人会主动回复当前状态。
