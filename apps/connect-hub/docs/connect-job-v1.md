...
旧版 Daily Paper 尚未实现该端点时，Connect Hub 会停止等待并把任务记录为 `cancel_failed`，不会谎报为 `cancelled`；这是 Daily Paper 模块接入 v1 时必须补齐的能力。

Connect Hub 会把受控子进程的 PID 和可执行文件写入 `jobs`。进程正常退出后清空运行态 PID；如果 Hub 在中途退出，下次启动会先核验 PID/可执行文件，再终止仍存在的独立进程组，随后把任务标为 `interrupted`。第一版只清理、不恢复任务。
...
```text
/jobs            # 当前会话最近 10 个任务
/job <短ID>      # 状态、事件、产物、错误与调用统计
/cancel          # 取消当前会话最近的活动任务
```
