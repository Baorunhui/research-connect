...
class ConferenceWorkflowAndUiTest(unittest.TestCase):
    ...
    def test_frontend_triggers_conference_retrieval_workflow(self):
        ...
        self.assertIn("isLocalDebugPage", runner)
        self.assertIn("192\\.168", runner)
        self.assertTrue((root / "src" / "local_server.py").exists())
    ...
    def test_index_html_no_longer_references_secret_session(self):
        root = pathlib.Path(__file__).resolve().parents[1]
        html = (root / "index.html").read_text(encoding="utf-8")
        self.assertNotIn("secret.session.js", html)
        self.assertNotIn("secret-gate-overlay", html)
