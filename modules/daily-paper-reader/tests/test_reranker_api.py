...
class FakeServerErrorResponse:
    status_code = 500
    text = '{"error":{"message":"Internal server error","type":"internal_server_error"}}'
...
class RerankerApiTest(unittest.TestCase):
    ...
    def test_siliconflow_reranker_retries_server_error(self):
        session = FakeSession([FakeServerErrorResponse(), FakeResponse()])
        reranker = self.api_mod.SiliconFlowReranker(
            api_key="test-key",
            base_url="https://example.test/v1/rerank",
            max_retries=2,
    ...
    def test_retryable_error_covers_5xx_and_rate_limit(self):
        mod = self.api_mod
        rate_limited = FakeRateLimitedResponse()
        server_err = FakeServerErrorResponse("boom")
        ok = FakeResponse()

        self.assertTrue(mod.SiliconFlowReranker._is_retryable_error(rate_limited, rate_limited.text))
        self.assertTrue(mod.SiliconFlowReranker._is_retryable_error(server_err, server_err.text))
        self.assertFalse(mod.SiliconFlowReranker._is_retryable_error(ok, ok.text))
