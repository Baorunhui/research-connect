...
* Survey Reports
  * 2026-08-27 · 多模态安全综述 <!--dpr-survey:abc123def0-->
    * <a class="dpr-sidebar-item-link dpr-sidebar-item-structured" href="#/survey/survey-abc123def0" data-sidebar-item="{&quot;title&quot;:&quot;多模态安全综述&quot;,&quot;tags&quot;:[{&quot;kind&quot;:&quot;label&quot;,&quot;label&quot;:&quot;综述&quot;}],&quot;published&quot;:&quot;2026-08-27&quot;,&quot;selection_source&quot;:&quot;survey_pipeline&quot;}">多模态安全综述</a>
...
function testSurveyReportsGroupParsesAndRenders() {
  const sidebar = loadSidebarForTest('#/survey/survey-abc123def0');
  const tools = sidebar.__test;
  const model = tools.parseSidebar(sampleSidebar);

  // 解析：块 id 来自 dpr-survey 标记，论文行 section='survey'
  assert.ok(Array.isArray(model.surveys), 'model.surveys should be an array');
  assert.equal(model.surveys.length, 1);
  const block = model.surveys[0];
  assert.equal(block.id, 'abc123def0');
  assert.equal(block.label, '2026-08-27 · 多模态安全综述');
  assert.equal(block.papers.length, 1);
  assert.equal(block.papers[0].section, 'survey');
  assert.equal(block.papers[0].href, '#/survey/survey-abc123def0');
  assert.equal(block.papers[0].tags[0].label, '综述');
  assert.equal(block.papers[0].published, '2026-08-27');
  assert.equal(
    tools.findCurrentPaperHrefFromModel(model, '#/survey/survey-abc123def0'),
    '#/survey/survey-abc123def0',
  );
  // 已读汇总包含综述条目
  const summary = tools.computeModelReadSummary(model, {});
  assert.equal(summary.total.papers, 6);

  // 渲染：默认收起；展开后含报告链接与标题
  const collapsed = tools.renderBodyHtml(model, {});
  assert.ok(collapsed.includes('data-panel="survey"'));
  assert.ok(collapsed.includes('综述报告'));
  assert.ok(/data-panel-toggle="survey" aria-expanded="false"/.test(collapsed));
  // 收起态 section 不得带 is-expanded（否则 CSS 会提前展开内容）
  assert.ok(!/dpr-sidebar-group-survey is-expanded/.test(collapsed));
  const expanded = tools.renderBodyHtml(model, {
    expandedGroups: { conference: true, daily: true, survey: true },
  });
  assert.ok(/data-panel-toggle="survey" aria-expanded="true"/.test(expanded));
  // 展开态 section 必须带 is-expanded，否则面板内容保持 display:none（回归锁：
  // 曾因 section 类名写死导致 aria-expanded=true 但内容永远不可见）
  assert.ok(/dpr-sidebar-group-survey is-expanded/.test(expanded));
  assert.ok(expanded.includes('href="#/survey/survey-abc123def0"'));
  assert.ok(expanded.includes('多模态安全综述'));
  // 综述面板位于会议论文面板之前
  assert.ok(expanded.indexOf('data-panel="survey"') < expanded.indexOf('data-panel="conference"'));

  // 搜索/未读筛选模式不渲染综述面板（综述不进入搜索索引）
  const searchHtml = tools.renderBodyHtml(model, {
    expandedGroups: { conference: true, daily: true, survey: true },
    search: 'rl',
  });
  assert.ok(!searchHtml.includes('data-panel="survey"'));
}
...
function testDailyAxisTabsShowDeepQuickBreakdown() {
  const sidebar = loadSidebarForTest('#/202606/24/paper-a');
  const tools = sidebar.__test;
  const model = tools.parseSidebar(sampleSidebar);

  const tagView = tools.buildDailyCalendarTagView(model, '20260624', '', {}, '');
  const tabsByKey = {};
  (tagView.tabs || []).forEach((tab) => {
    tabsByKey[tab.key] = tab;
  });
  // 「全部」= 精读1 + 速读1；词条 rl 只有精读区的 paper-a
  assert.deepEqual([tabsByKey['__all__'].deepCount, tabsByKey['__all__'].quickCount], [1, 1]);
  assert.deepEqual([tabsByKey['rl'].deepCount, tabsByKey['rl'].quickCount], [1, 0]);
  ...
  assert.ok(html.includes('class="dpr-sidebar-axis-tab-kinds"'));
  assert.ok(html.includes('<span class="dpr-sidebar-axis-tab-kind-deep">精1</span>'));
  assert.ok(html.includes('<span class="dpr-sidebar-axis-tab-kind-quick">速1</span>'));

  // 会议论文轴没有精读/速读概念，不得渲染拆分角标
  const dailyRowStart = html.indexOf('data-axis-group="daily"');
  const conferenceRegion = html.slice(html.indexOf('data-axis-group="conference"'), dailyRowStart);
  assert.ok(!conferenceRegion.includes('dpr-sidebar-axis-tab-kind'));

  // 未读原地刷新走同一批 view 构建器，拆分计数必须继续存在
  const refreshed = tools.buildAxisViewForMode(model, 'daily', 'date', {
    dailyViewMode: 'date',
    activeDailyDate: '20260624',
  }, {});
  const refreshedAllTab = (refreshed.tabs || []).find((tab) => tab.key === '__all__');
  assert.ok(refreshedAllTab && refreshedAllTab.deepCount === 1 && refreshedAllTab.quickCount === 1);

  // CSS 合同：三种新类名必须有真实规则
  const css = fs.readFileSync('app/app.css', 'utf8');
  cssRule(css, '.dpr-sidebar-axis-tab-kinds');
  cssRule(css, '.dpr-sidebar-axis-tab-kind-deep');
  cssRule(css, '.dpr-sidebar-axis-tab-kind-quick');
}

function testDailyTagMultiSelectCombination() {
  const sidebar = loadSidebarForTest('#/202606/24/paper-a');
  const tools = sidebar.__test;
  const model = tools.parseSidebar(sampleSidebar);
  // fixture：0624 → paper-a(精读, rl) + paper-b(速读, 未标注)；0623 → paper-d(精读, rl)

  // 单标签仍接受旧字符串入参（向后兼容）
  const single = tools.buildDailyCalendarTagView(model, '20260624', 'rl', {}, '');
  assert.deepEqual(single.selectedKeys, ['rl']);
  assert.equal(single.activeKey, 'rl');

  // 多选并集：rl + 未标注 两枚标签同时生效
  const multi = tools.buildDailyCalendarTagView(model, '20260624', ['rl', '未标注'], {}, '');
  assert.equal(multi.activeKey, 'rl|未标注');
  assert.deepEqual(multi.selectedKeys.slice().sort(), ['rl', '未标注'].sort());
  const groupedPapers = [];
  (multi.groups || []).forEach((g) => (g.papers || []).forEach((p) => groupedPapers.push(p)));
  assert.equal(groupedPapers.length, 2, 'union keeps papers of both tags');

  // 'a|b' 组合串也能作为入参
  const joined = tools.buildDailyCalendarTagView(model, '20260624', 'rl|未标注', {}, '');
  assert.equal(joined.activeKey, 'rl|未标注');
  const joinedPapers = [];
  (joined.groups || []).forEach((g) => joinedPapers.push(...(g.papers || [])));
  assert.equal(joinedPapers.length, 2);

  // 空选 = 全部
  const none = tools.buildDailyCalendarTagView(model, '20260624', [], {}, '');
  assert.equal(none.activeKey, '__all__');
  assert.deepEqual(none.selectedKeys, []);

  // 渲染：两枚 chip 同时高亮、「全部」熄灭、出现命中徽标
  const html = tools.renderBodyHtml(model, {
    expandedGroups: { conference: true, daily: true },
    conferenceViewMode: 'conf',
    dailyViewMode: 'tag',
    activeConference: 'neurips-2024',
    activeDailyDate: '20260624',
    activeDailyTag: 'rl|未标注',
    readMap: {},
  });
  const dailyRegion = html.slice(html.indexOf('data-axis-group="daily"'));
  const activeChipsInDaily = dailyRegion.split('class="dpr-sidebar-axis-tab is-active"').length - 1;
  assert.equal(activeChipsInDaily, 2, 'both selected tag chips highlighted');
  assert.ok(dailyRegion.includes('<span class="dpr-sidebar-axis-matched">2 篇</span>'), 'matched badge present');

  // 组合筛选勾选不破坏精读/速读角标语义
  assert.ok(dailyRegion.includes('dpr-sidebar-axis-tab-kind-deep'));
}
...
function testQuickLinksCenterTextAndDetachIcon() {
  ...
  assert.ok(homeIndex >= 0 && tutorialIndex > homeIndex);
  assert.ok(headerHtml.includes('aria-hidden="true">📖</span>教程</span>'));
  assert.ok(headerHtml.includes('dpr-sidebar-quick-summarize'), 'summarize quick link should be present');
  assert.ok(headerHtml.includes('dpr-sidebar-quick-survey'), 'survey quick link should be present');
  assert.ok(headerHtml.includes('#/survey'), 'survey quick link should point to #/survey');
...
function testPanelCountsUseVisibleAxisSlice() {
  ...
  assert.deepEqual(summary.total, { papers: 6, unread: 4 });
...
testSurveyReportsGroupParsesAndRenders();
testAxisViewsForDailyAndConference();
testHyphenatedConferenceMarkerParsing();
testAxisTabsRenderUnreadCounts();
testDailyAxisTabsShowDeepQuickBreakdown();
testDailyTagMultiSelectCombination();
