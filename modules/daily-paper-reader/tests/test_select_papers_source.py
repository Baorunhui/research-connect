...
class SelectPapersDeepPriorityModeTest(unittest.TestCase):
    """精读区语义：≥8 分全部入选（无上限）；deep_base 是保底下限，不足时从 6 分以上回填。"""
    ...
    def test_process_mode_keeps_all_eight_plus_without_cap(self):
        candidates = [
        ...
        result = self.mod.process_mode(
            ...
            tag_count=1,
            mode="standard",
            cfg={"deep_base": 1, "deep_strategy": "round_robin"},
            carryover_ratio=0.5,
        )
        self.assertEqual(result.get("stats", {}).get("deep_selected"), 5)
        self.assertIsNone(result.get("stats", {}).get("deep_cap"))
        deep_ids = [item.get("id") for item in result.get("deep_dive", [])]
        self.assertEqual(deep_ids, ["p-1", "p-2", "p-3", "p-4", "p-5"])
        self.assertEqual(result.get("stats", {}).get("deep_backfilled"), 0)

    def test_process_mode_backfills_guarantee_from_six_to_eight(self):
        candidates = [
            {"id": "p-1", "llm_score": 8.9},
            {"id": "p-2", "llm_score": 7.9},
            {"id": "p-3", "llm_score": 7.5},
            {"id": "p-4", "llm_score": 6.1},
            {"id": "p-5", "llm_score": 5.9},
        ]
        result = self.mod.process_mode(
            candidates=candidates,
            tag_count=1,
            mode="standard",
            cfg={"deep_base": 4, "deep_strategy": "score"},
            carryover_ratio=0.5,
        )
        stats = result.get("stats", {})
        self.assertEqual(stats.get("deep_selected"), 4)
        self.assertEqual(stats.get("deep_backfilled"), 3)
        deep_ids = [item.get("id") for item in result.get("deep_dive", [])]
        self.assertEqual(deep_ids, ["p-1", "p-2", "p-3", "p-4"])
        # 5.9 分不跌破速读线，不回填进精读，也不进速读
        quick_ids = [item.get("id") for item in result.get("quick_skim", [])]
        self.assertNotIn("p-5", quick_ids)

    def test_process_mode_backfill_never_pulls_below_six(self):
        candidates = [
            {"id": "p-1", "llm_score": 8.2},
            {"id": "p-2", "llm_score": 5.5},
            {"id": "p-3", "llm_score": 4.0},
        ]
        result = self.mod.process_mode(
            candidates=candidates,
            tag_count=1,
            mode="standard",
            cfg={"deep_base": 5, "deep_strategy": "score"},
            carryover_ratio=0.5,
        )
        stats = result.get("stats", {})
        self.assertEqual(stats.get("deep_selected"), 1)
        self.assertEqual(stats.get("deep_backfilled"), 0)
        deep_ids = [item.get("id") for item in result.get("deep_dive", [])]
        self.assertEqual(deep_ids, ["p-1"])
