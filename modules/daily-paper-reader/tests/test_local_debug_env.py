...
class LocalDebugEnvTest(unittest.TestCase):
    ...
    def test_local_conference_command_skips_existing_topic_before_pipeline(self):
        ...
        # 新命令形态：[python, "-c", <orchestrator 源码>, <params JSON>]，不再依赖 bash。
        self.assertEqual(cmd[0], self.mod.sys.executable)
        self.assertEqual(cmd[1], "-c")
        script = cmd[2]
        params = cmd[3]

        # 编排器必须在跑 pipeline 之前完成 topic marker 计算与 sidebar 去重判定。
        self.assertIn("build_conference_topic_marker", script)
        self.assertIn("topic_from_profile_tag", script)
        ...
        # profile_tag 通过 params 传入并被写进环境变量 DPR_FILTER_PROFILE_TAG
        self.assertIn('"profile_tag": "RL"', params)
        self.assertIn("env['DPR_FILTER_PROFILE_TAG'] = profile_tag", script)
        # 去重判定必须早于 pipeline 调用
        self.assertLess(script.index("sys.exit(0)"), script.index("conference_pipeline.py"))
        self.assertLess(script.index("topic_marker"), script.index("conference_pipeline.py"))
        # 不再生成 bash 脚本
