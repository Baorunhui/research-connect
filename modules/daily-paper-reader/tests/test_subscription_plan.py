...
class SubscriptionPlanTest(unittest.TestCase):
    ...
    def test_tag_only_profile_falls_back_to_tag_recall(self):
        # 用户只填一个标签、不填关键词/意图查询时，用标签本身兜底召回，避免日报 0 篇。
        # 兜底查询类型为 intent_query，确保能进入 rerank/精排链路拿到星级与相关性分。
        cfg = {
            'subscriptions': {
                'intent_profiles': [
                    {'tag': 'Pose Estimation', 'description': '', 'enabled': True, 'keywords': [], 'intent_queries': []},
                ],
            }
        }
        plan = build_pipeline_inputs(cfg)
        self.assertTrue(plan['bm25_queries'], 'tag-only profile should produce a fallback bm25 query')
        self.assertTrue(plan['embedding_queries'], 'tag-only profile should produce a fallback embedding query')
        self.assertEqual(plan['bm25_queries'][0]['type'], 'intent_query')
        self.assertEqual(plan['bm25_queries'][0]['query_text'], 'Pose Estimation')
        self.assertEqual(plan['embedding_queries'][0]['type'], 'intent_query')
        self.assertEqual(plan['embedding_queries'][0]['query_text'], 'Pose Estimation')

    def test_tag_only_profile_prefers_description_as_fallback(self):
        cfg = {
            'subscriptions': {
                'intent_profiles': [
                    {'tag': 'RAG', 'description': 'Retrieval-Augmented Generation', 'enabled': True, 'keywords': [], 'intent_queries': []},
                ],
            }
        }
        plan = build_pipeline_inputs(cfg)
        self.assertEqual(plan['bm25_queries'][0]['query_text'], 'Retrieval-Augmented Generation')

    def test_profile_with_keywords_does_not_use_tag_fallback(self):
        cfg = {
            'subscriptions': {
                'intent_profiles': [
                    {'tag': 'RAG', 'enabled': True, 'keywords': [{'keyword': 'rag', 'query': 'rag', 'enabled': True}], 'intent_queries': []},
                ],
            }
        }
        plan = build_pipeline_inputs(cfg)
        # 有关键词的词条走正常 keyword 类型，不触发兜底
        types = {q['type'] for q in plan['bm25_queries']}
