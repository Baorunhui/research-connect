...
def _build_from_profiles(subs: Dict[str, Any], known_sources: List[str]) -> Dict[str, Any]:
  ...
  for profile in profiles:
    ...
    # 兜底召回：词条只填了标签（无关键词、无意图查询）时，用标签/描述本身作为意图查询，
    # 让“只填一个标签”也能产出论文。类型用 intent_query 以便 rerank/精排链路正常处理。
    if not profile.get("keywords") and not profile.get("intent_queries"):
      fallback_text = _norm_text(profile.get("description") or "") or tag
      bm25_queries.append(
        {
          "type": "intent_query",
          "tag": tag,
          ...
          "query_terms": [{"text": fallback_text, "weight": MAIN_TERM_WEIGHT}],
          "boolean_expr": "",
          "logic_cn": "",
          "source": "tag_fallback",
          "or_soft_weight": OR_SOFT_WEIGHT,
      ...
      context_keywords.append({"tag": paper_tag_keyword, "keyword": fallback_text, "logic_cn": ""})
      context_queries.append({"tag": paper_tag_query, "query": fallback_text, "logic_cn": ""})
