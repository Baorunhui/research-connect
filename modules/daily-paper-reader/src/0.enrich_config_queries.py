...
def call_llm_json(client: OpenAIClient, messages: List[Dict[str, str]], schema_name: str, schema: Dict[str, Any]) -> Dict[str, Any]:
...
def main() -> None:
    ...
    if not api_key:
        raise RuntimeError("缺少 LLM_API_KEY/DEEPSEEK_API_KEY/SUMMARY_API_KEY 环境变量，无法调用 LLM。")
    ...
    client = OpenAIClient(api_key=api_key, model=MODEL_NAME, base_url=BASE_URL)
