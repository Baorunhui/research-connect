...
try:
    from figure_interpretation import figures_enabled, interpret_paper_figures
except Exception:  # pragma: no cover
    from src.figure_interpretation import figures_enabled, interpret_paper_figures

try:
    from venue_enrichment import enrich_venue
except Exception:  # pragma: no cover
...
# LLM 配置（使用 llm.py 内统一的 OpenAI 兼容客户端）
DEEPSEEK_API_KEY = resolve_llm_api_key()
DEEPSEEK_BASE_URL = resolve_llm_base_url()
DEEPSEEK_MODEL = resolve_llm_model()
STEP6_STRUCTURED_MAX_TOKENS = int(os.getenv("DPR_LLM_MAX_OUTPUT_TOKENS") or 16 * 1024)
...
def _jina_timeout() -> int:
    """单次 Jina 请求超时秒数，默认 15，可用 DPR_JINA_TIMEOUT 覆盖。"""
    try:
        return max(5, int(os.getenv("DPR_JINA_TIMEOUT") or "15"))
    except Exception:
        return 15


def _jina_disabled() -> bool:
    """置 1/off 外均可关闭 Jina 全文抓取（直接走 PyMuPDF 兜底）。"""
    return str(os.getenv("DPR_JINA_DISABLED") or "").strip().lower() in {"1", "true", "yes", "on"}


def _jina_max_retries() -> int:
    try:
        return max(0, int(os.getenv("DPR_JINA_MAX_RETRIES") or "3"))
    except Exception:
        return 3


def fetch_paper_markdown_via_jina(pdf_url: str, max_retries: int | None = None) -> str | None:
    if not pdf_url or _jina_disabled():
        return None
    if max_retries is None:
        max_retries = _jina_max_retries()
    if max_retries <= 0:
        return None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            log(f"[JINA] 第 {attempt} 次请求：{full_url}（timeout={timeout}s）")
...
def ensure_home_module_files(
    ...
    notice_path = prepare_home_module_paths(docs_dir, home_template_dir)
...
def build_paper_fallback_text(paper: Dict[str, Any]) -> str:
    """全文抓取失败时的内容兜底：优先用 LLM 生成的中文速览/摘要，其次英文摘要。"""
    parts = []
    tldr = str(paper.get("llm_tldr_cn") or paper.get("llm_tldr") or paper.get("llm_tldr_en") or "").strip()
    if tldr:
        parts.append("### 概述（LLM 生成）###\n" + tldr)
    evidence = str(paper.get("llm_evidence_cn") or paper.get("llm_evidence") or paper.get("llm_evidence_en") or "").strip()
    if evidence:
        parts.append("### 相关证据 ###\n" + evidence)
    abstract = str(paper.get("abstract") or "").strip()
    if abstract:
        label = "### 摘要（Abstract）###\n" + abstract
        parts.append(label)
    zh = str(paper.get("title_zh") or "").strip()
    title = str(paper.get("title") or "").strip()
    header = "\n".join([f"# {p}".strip() for p in (zh, title) if p])
    return "\n\n".join(part for part in ([header] if header else []) + parts)


def ensure_text_content(pdf_url: str, txt_path: str, fallback: str = "") -> str:
    ...
    if text_content is None and pdf_url:
        try:
            ...
            log(f"[WARN] PDF 抓取失败（{pdf_url}）：{exc}，将回退到摘要/元数据文本。")
            text_content = None
    if not (text_content or "").strip():
        # 全文抓取（Jina / PDF）均失败时，回退到本地已有的摘要/元数据，
        # 避免日报因外网抓取被阻断而产出空论文页。
        text_content = (fallback or "").strip()
        if text_content:
            log("[INFO] 全文抓取失败，已用论文摘要/元数据作为内容兜底。")
...
def maybe_generate_paper_media(
    ...
    *,
    ...
    # 未知来源且没有现成 PDF 字节时无法定位原文，跳过抽图；带字节时 source_key
    # 只影响资产目录命名，照常抽取（上传 PDF 总结的 source 可为 pdf/conference 等）。
    if source_key not in {"arxiv", "biorxiv"} and pdf_bytes is None:
        return [], []
...
def apply_figure_interpretation(
    paper: Dict[str, Any],
    *,
    docs_dir: str,
    section: str,
    ...
    """
    对精读区论文的图表做纯文本解读（不读图），结果写入 paper['_figure_assets']/['_table_assets'] 的 interpretation。
    ...
    def _done(it: Dict[str, Any]) -> bool:
        return bool(str(it.get("interpretation") or "").strip() or it.get("interpretation_skipped"))

    # 全部已有解读（或无上下文跳过）则无需重复解读（避免重复计费）
    if all(_done(f) for f in figures) and all(_done(t) for t in tables):
        return
    full_text = ""
    if txt_path and os.path.exists(txt_path):
        try:
            ...
            full_text = ""
    try:
        new_figures, new_tables = interpret_paper_figures(
            figures,
            tables,
            paper,
            docs_dir,
        ...
        if new_figures:
            paper["_figure_assets"] = new_figures
        if new_tables:
            paper["_table_assets"] = new_tables
    except Exception as e:
        log(f"[WARN] 图表解读失败（跳过）：{e}")
...
def build_markdown_content(
    ...
    venue_status = str(paper.get("venue_status") or "").strip()
    authoritative_url = str(paper.get("authoritative_url") or "").strip()
    if venue:
        lines.append(f"venue: {yaml_escape_value(venue)}")
    if venue_status:
        lines.append(f"venue_status: {yaml_escape_value(venue_status)}")
    if authoritative_url:
        lines.append(f"authoritative_url: {yaml_escape_value(authoritative_url)}")
    if figure_assets:
...
def process_paper(
    ...
    _ = enrich_venue(paper, config)  # 可选权威来源校验层（受 config 开关控制，默认关）
    title = (paper.get("title") or "").strip()
    ...
    if os.path.exists(md_path):
        ...
        if glance_only and pdf_url:
            try:
                ensure_text_content(pdf_url, txt_path, fallback=build_paper_fallback_text(paper))
            except Exception:
        ...
        if section == "deep" and figures_enabled():
            if not isinstance(paper.get("_figure_assets"), list) and not isinstance(paper.get("_table_assets"), list):
                ...
                if fm:
                    for key, attr in (("figures_json", "_figure_assets"), ("tables_json", "_table_assets")):
                        raw = str(fm.get(key) or "").strip()
                        if raw:
                            try:
                                paper[attr] = json.loads(raw)
                            except Exception:
                                pass
            # 先确保全文就绪（幂等），作为图表解读的正文引用上下文
            _pdf_url = str(paper.get("pdf_url") or paper.get("link") or "").strip()
            if _pdf_url:
                try:
                    ensure_text_content(_pdf_url, txt_path, fallback=build_paper_fallback_text(paper))
                except Exception:
                    pass
            before_figs = json.dumps(paper.get("_figure_assets", []), ensure_ascii=False)
            before_tabs = json.dumps(paper.get("_table_assets", []), ensure_ascii=False)
            apply_figure_interpretation(paper, docs_dir=docs_dir, section=section, txt_path=txt_path, client=paper_llm_client)
            after_figs = json.dumps(paper.get("_figure_assets", []), ensure_ascii=False)
            after_tabs = json.dumps(paper.get("_table_assets", []), ensure_ascii=False)
            if after_figs != before_figs and isinstance(paper.get("_figure_assets"), list):
                updated, _ = upsert_front_matter_field(
                    existing, "figures_json", yaml_escape_value(after_figs)
                )
                if updated != existing:
                    with open(md_path, "w", encoding="utf-8") as f:
                        f.write(updated + ("\n" if not updated.endswith("\n") else ""))
                    existing = updated
            if after_tabs != before_tabs and isinstance(paper.get("_table_assets"), list):
                updated, _ = upsert_front_matter_field(
                    existing, "tables_json", yaml_escape_value(after_tabs)
                )
                if updated != existing:
                    with open(md_path, "w", encoding="utf-8") as f:
                        f.write(updated + ("\n" if not updated.endswith("\n") else ""))
...
def generate_external_paper_docs(
    paper: Dict[str, Any],
    *,
    date_str: str,
    docs_dir: str,
    ...
    """为站外来源的论文复用日报尾段（速览 + 深度总结 + 图表解读 + 展示）。

    与 process_paper 同源，但面向「单篇 / 站外」论文：用和日报纸张页完全相同的数据结构与
    渲染产出 `docs/<日期>/<id>-<slug>.md`，使得「论文总结」功能落地成一张与日报数据结构
    统一的独立纸张，可被 Sidebar / 未读等机制一并管理。

    Args:
        paper: 已归一化为日报字段的论文 dict（至少 title/abstract/id/authors/pdf_url/source）。
        date_str: YYYYMMDD（决定 docs/<日期>/ 子目录）。
        docs_dir: docs 根目录。
        section: 'deep'（精读区，生成详细总结）或 'quick'（速读区，仅速览+摘要）。
        pdf_bytes: 可选，已下载的 PDF 字节。提供时跳过下载，直接用于抽图（如 PDF 上传场景）。
        full_text: 可选，论文全文文本。提供时写入 txt 供图表解读/详细总结引用（如 PDF 上传场景）。
        on_progress: 可选回调 (stage, message)，用于向调用方（如本地总结 job）发细粒度进度事件。
            回调异常会向上传播（如 job 取消信号），调用方需自行保证回调健壮。

    Returns:
        (paper_id, title, md_path)：
        - paper_id 形如 '<date>/<basename>'，可直接用于 Sidebar href（#/<paper_id>）。
        - title：论文标题。
        - md_path：生成/更新的 markdown 绝对路径。
    """
    ...
    if section == "deep" and figures_enabled():
        ...
        if pdf_url:
            try:
                ...
                pass
        elif full_text and not os.path.exists(txt_path):
            try:
                os.makedirs(os.path.dirname(txt_path), exist_ok=True)
                with open(txt_path, "w", encoding="utf-8") as f:
                    f.write(full_text)
    ...
    _progress("translate", "正在翻译标题与摘要")
    zh_title, zh_abstract = translate_title_and_abstract_to_zh(
        title, abstract_en, client=paper_llm_client
    )
    _progress("translate", "翻译完成")
    tags_list = build_tags_list(section, paper.get("llm_tags") or [])
    content = build_markdown_content(paper, section, zh_title, zh_abstract, tags_list)
...
def update_sidebar(
    ...
    def build_sidebar_item_payload(
        ...
        # 优先使用显式传入的 link（如外部 PDF/网页无 arXiv 关系时）；
        # 否则回退到既有推断逻辑——把 route 最后一段视为 arXiv id 拼出 arxiv.org 链接，
        # 最后回退到 route_href（相对哈希路由，仅在目标也存在该路径时）。
        if explicit_link:
            paper_link = explicit_link
        else:
    ...
    if deep_entries or extra_deep:
        ...
        for paper_id, title, tags in deep_entries:
            ...
            explicit_link = link_by_id.get(str(paper_id).strip(), "")
            payload_json = build_sidebar_item_payload(paper_id, title, tags, href, evidence, explicit_link)
            block.append(
                "      * "
...
def build_home_readme_content(
    ...
    notice_path = ensure_home_module_files(docs_dir, home_template_dir)
...
def _parse_generated_md_to_meta(
    ...
    venue_value = str(fm_meta.get("venue") or "").strip()
    venue_status_value = str(fm_meta.get("venue_status") or "").strip()
    authoritative_url_value = str(fm_meta.get("authoritative_url") or "").strip()
    ...
    return {
        ...
        "venue": venue_value,
        "venue_status": venue_status_value,
