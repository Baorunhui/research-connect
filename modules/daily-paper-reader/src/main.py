...
def resolve_recall_mode(config: dict | None = None) -> str:
    """召回模式：config local.recall.mode > 环境变量 DPR_RECALL_MODE > "supabase"。

    - supabase：既有云端链路（Step 1/2.1/2.2，依赖 Supabase RPC）
    - local：本地链路（Step 1L arXiv API 增量抓取 + Step 2L 本地 FTS/embedding），
      全程不依赖 Supabase，lane 输出与 2.1/2.2 同构，2.3 起下游完全一致。
    """
    if config is None:
        config = _load_full_config()
    raw = ""
    local_section = (config or {}).get("local") or {}
    if isinstance(local_section, dict):
        raw = str((local_section.get("recall") or {}).get("mode") or "").strip().lower()
    if not raw:
        raw = str(os.getenv("DPR_RECALL_MODE") or "").strip().lower()
    return raw if raw in ("supabase", "local") else "supabase"
...
def resolve_run_date_token(fetch_days: int | None, run_date: str | None = None) -> str:
    """
    统一运行日期标识：
    - 显式 --run-date YYYYMMDD 时使用该单日 token（用于补跑历史某天的日报）
...
def _main_impl() -> None:
    ...
    parser.add_argument(
        "--run-date",
        default="",
        help="强制使用指定单日 token（YYYYMMDD）作为运行日期，用于补跑历史某天的日报。"
             "留空则按 fetch_days / config 自动决定。",
    )
    parser.add_argument(
        "--fetch-mode",
        default="auto",
    ...
    recall_mode = resolve_recall_mode()
    ran_local = False
    if recall_mode == "local":
        # 本地召回模式：窗口数据走 arXiv API 增量抓取 + 本地 SQLite FTS/embedding，
        # 全程不依赖 Supabase；lane 输出与 2.1/2.2 同构，2.3 及下游原样复用。
        print("[INFO] 召回模式：local（arXiv API 增量抓取 + 本地 FTS5 + embedding 精排）", flush=True)
        try:
            run_step(
                "Step 1L - 本地增量抓取",
                [python, os.path.join(SRC_DIR, "fetch_arxiv_window.py")],
            )
            run_step(
                "Step 2L - 本地召回",
                [python, os.path.join(SRC_DIR, "recall_local.py")],
            )
            ran_local = True
        except subprocess.CalledProcessError as exc:
            # 兜底：arXiv 不可达且本地库为空时，自动回退云端链路，日报不中断
            print(
                f"[WARN] 本地召回失败（exit={exc.returncode}），"
                "自动回退云端（Supabase）链路——本地库有数据后下次运行会自动走回本地。",
                flush=True,
            )
    if not ran_local:
        if recall_mode == "local":
            print("[INFO] 本次以云端（Supabase）链路完成召回。", flush=True)
