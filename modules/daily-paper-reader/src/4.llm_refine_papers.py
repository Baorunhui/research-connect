...
from llm import OpenAIClient, resolve_llm_api_key, resolve_llm_base_url, resolve_llm_model, resolve_max_output_tokens
...
DEFAULT_FILTER_MODEL = (
    ...
    or resolve_llm_model()
    or os.getenv("SUMMARY_MODEL")
    or os.getenv("DEEPSEEK_MODEL")
    or "deepseek-v4-flash"
)
DEFAULT_DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL") or os.getenv("SUMMARY_BASE_URL") or resolve_llm_base_url()
...
def _make_filter_client(api_key: str, model: str, max_output_tokens: int) -> OpenAIClient:
    client = OpenAIClient(api_key=api_key, model=model, base_url=DEFAULT_DEEPSEEK_BASE_URL)
...
def process_file(
    ...
    if not api_key:
        raise RuntimeError("missing LLM_API_KEY/DEEPSEEK_API_KEY/SUMMARY_API_KEY")
