...
MODES = {
    "standard": {
        ...
        # deep_base 是精读保底下限：≥8 分全部进入精读区（无上限），不足保底时从 6 分以上回填
        "deep_base": 5,
    },
    "extend": {
        "quick_base": 15,
...
CARRYOVER_DAYS = 5
CARRYOVER_RATIO = 0.5
MIN_DEEP_DIVE_BASE = 3
# 保底回填的分数下限：精读保底只从速读线（6 分）以上回填，不把噪声硬凑进精读区
DEEP_GUARANTEE_MIN_SCORE = 6.0
...
def load_recommend_setting() -> Dict[str, Any]:
    """读取设置面板保存的推荐名额覆盖（config.yaml 顶层 recommend_setting 段）。"""
    if not os.path.exists(CONFIG_FILE):
        return {}
    try:
        import yaml  # type: ignore
        ...
        log(f"[WARN] failed to read recommend_setting: {exc}")
        return {}
    setting = data.get("recommend_setting") if isinstance(data, dict) else None
    return setting if isinstance(setting, dict) else {}


def _parse_int_setting(value: Any) -> int | None:
...
def apply_recommend_setting(mode_cfg: Dict[str, Any], mode: str) -> Dict[str, Any]:
    """把用户自定义名额合并进模式配置；空值/缺省键保持模式默认。

    - deep_dive_base > 0 时覆盖精读保底数量，并夹到最小 MIN_DEEP_DIVE_BASE；
      （≥8 分论文不受名额限制全部进入精读区，保底不足时从 6 分以上回填）
    - deep_dive_unlimited 已废弃：精读默认不限量，该开关仅保留兼容不再生效；
    - quick_skim_base > 0 时覆盖速读基数（tag_count 加成逻辑不变）；
    - skims 模式没有精读区，仅 quick_skim_base 生效。
    """
    setting = load_recommend_setting()
    if not setting:
        return mode_cfg
    cfg = dict(mode_cfg or {})
    if mode != "skims":
        base = _parse_int_setting(setting.get("deep_dive_base"))
        if base is not None and base > 0:
            cfg["deep_base"] = max(base, MIN_DEEP_DIVE_BASE)
        if bool(setting.get("deep_dive_unlimited")):
            cfg["deep_unlimited"] = True
    quick_base = _parse_int_setting(setting.get("quick_skim_base"))
    if quick_base is not None and quick_base > 0:
        cfg["quick_base"] = quick_base
    return cfg
...
def process_mode(
    ...
    # 上限不设：≥8 分的论文全部进入精读区（原“不限量”成为默认行为）
    deep_selected = list(deep_candidates)

    # 保底下限：不足 deep_base 时，从 6~8 分按分数从高到低回填，不跌破 DEEP_GUARANTEE_MIN_SCORE
    deep_base = int(cfg.get("deep_base") or 0)
    backfilled = 0
    if deep_base > 0 and len(deep_selected) < deep_base:
        selected_ids = {p.get("id") for p in deep_selected}
        backfill_pool = [
            p
            for p in candidates
            if p.get("id") not in selected_ids
            and DEEP_GUARANTEE_MIN_SCORE <= float(p.get("llm_score", 0)) < 8.0
        ]
        backfill_pool = sort_by_score(backfill_pool)
        backfill = backfill_pool[: deep_base - len(deep_selected)]
        deep_selected = deep_selected + backfill
    ...
    stats = {
        ...
        "deep_cap": None,
        "deep_guarantee": deep_base,
        "deep_backfilled": backfilled,
...
def main() -> None:
    ...
    for mode in modes:
        cfg = apply_recommend_setting(MODES.get(mode) or {}, mode)
        ...
        log(
            ...
            f"guarantee={stats.get('deep_guarantee')} backfilled={stats.get('deep_backfilled')} "
            f"target={stats.get('quick_skim_target')}"
        )
