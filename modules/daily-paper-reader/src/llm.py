...
# 统一 OpenAI 兼容模型配置的兜底值（可通过环境变量逐项覆盖）
DEFAULT_LLM_BASE_URL_ENV = "https://api.deepseek.com"
DEFAULT_LLM_MODEL_ENV = "deepseek-v4-flash"
...
class OpenAIClient(LLMClient):
    ...
    适用于任何暴露 /v1/chat/completions 且使用 Bearer 鉴权的 OpenAI 兼容服务。
    """
...
def parse_provider_model(model_str: str) -> Tuple[str, str]:
    ...
    - "datacenter/qwen-chat"       -> ("datacenter", "qwen-chat")
    """
...
class ClientFactory:
    ...
    def from_env():
        """
        基于环境变量创建具体客户端，支持任意 OpenAI 兼容服务。

        必填：
        - LLM_MODEL：形如 'provider/model'。
        选填（优先级从高到低，低优先级仅在高优先级缺失时兜底）：
        - LLM_API_KEY：通用 API key（优先级最高）
        - LLM_BASE_URL：通用 base_url
        - <PROVIDER>_API_KEY / <PROVIDER>_BASE_URL：provider 专属 key/base_url
        - DEEPSEEK_API_KEY / DEEPSEEK_BASE_URL：兼容旧配置
        - SUMMARY_API_KEY / SUMMARY_BASE_URL：兼容旧配置

        默认 base_url 为 OpenAI 兼容端点的公共 DeepSeek 地址。
        """
        ...
        provider_key_prefix = provider.upper()

        api_key = (
            (os.getenv('LLM_API_KEY') or '').strip()
            or (os.getenv(f'{provider_key_prefix}_API_KEY') or '').strip()
            or (os.getenv('DEEPSEEK_API_KEY') or '').strip()
            or (os.getenv('SUMMARY_API_KEY') or '').strip()
        ) or None

        base_url = (
            (os.getenv('LLM_BASE_URL') or '').strip()
            or (os.getenv(f'{provider_key_prefix}_BASE_URL') or '').strip()
            or (os.getenv('DEEPSEEK_BASE_URL') or '').strip()
            or (os.getenv('SUMMARY_BASE_URL') or '').strip()
        ) or DEFAULT_LLM_BASE_URL_ENV

        return OpenAIClient(api_key=api_key or '', model=model, base_url=base_url)
...
def _config_provider() -> str:
    """从 LLM_MODEL（'provider/model'）中取 provider 大写形式，未配置则为空串。"""
    model_env = (os.getenv('LLM_MODEL') or '').strip()
    if model_env and '/' in model_env:
        return parse_provider_model(model_env)[0].upper()
    return ''


def resolve_llm_api_key() -> str:
    """统一解析 OpenAI 兼容模型的 API Key。

    优先级：LLM_API_KEY > {PROVIDER}_API_KEY > DEEPSEEK_API_KEY > SUMMARY_API_KEY。
    """
    provider = _config_provider()
    return (
        (os.getenv('LLM_API_KEY') or '').strip()
        or (os.getenv(f'{provider}_API_KEY') if provider else '')
        or (os.getenv('DEEPSEEK_API_KEY') or '').strip()
        or (os.getenv('SUMMARY_API_KEY') or '').strip()
    )
...
def resolve_llm_base_url() -> str:
    """统一解析 OpenAI 兼容模型的 base_url。

    优先级：LLM_BASE_URL > {PROVIDER}_BASE_URL > DEEPSEEK_BASE_URL > SUMMARY_BASE_URL。
    """
    provider = _config_provider()
    return (
        (os.getenv('LLM_BASE_URL') or '').strip()
        or (os.getenv(f'{provider}_BASE_URL') if provider else '')
        or (os.getenv('DEEPSEEK_BASE_URL') or '').strip()
        or (os.getenv('SUMMARY_BASE_URL') or '').strip()
        or DEFAULT_LLM_BASE_URL_ENV
    )
...
def resolve_llm_model(default: str = DEFAULT_LLM_MODEL_ENV) -> str:
    """统一解析 OpenAI 兼容模型的模型名。

    优先级：LLM_MODEL 的 model 部分 > SUMMARY_MODEL > DEEPSEEK_MODEL。
    """
    model_env = (os.getenv('LLM_MODEL') or '').strip()
    if model_env and '/' in model_env:
        return parse_provider_model(model_env)[1]
    return (
        (os.getenv('SUMMARY_MODEL') or '').strip()
