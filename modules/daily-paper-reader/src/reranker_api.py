...
class SiliconFlowReranker:
    ...
    *,
    ...
    self.max_retries = max(
      int(
        ...
        else _env_int("SILICONFLOW_RERANK_MAX_RETRIES", 4)
      ),
      0,
    )
    ...
    *,
    ...
    for attempt in range(self.max_retries + 1):
      ...
      try:
        ...
        if attempt < self.max_retries and self._is_retryable_error(response, text):
          # 5xx（上游瞬时故障）用短退避，429 限流用完整 retry_delay_seconds
          if self._is_rate_limit_error(response, text):
            time.sleep(self.retry_delay_seconds)
          else:
            time.sleep(self.retry_delay_seconds * 0.2)
          continue
  ...
  @staticmethod
  def _is_retryable_error(response: requests.Response, text: str) -> bool:
    # new-api 类中转站（sinksilk / zwwen）对 rerank 常间歇性返回 5xx
    # 瞬时故障应退避重试，避免整条流水线中断。
    status_code = int(getattr(response, "status_code", 0) or 0)
    return (
      SiliconFlowReranker._is_rate_limit_error(response, text)
