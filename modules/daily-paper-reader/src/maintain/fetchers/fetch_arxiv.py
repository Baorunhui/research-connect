...
# 抓取分类（可被 config.yaml 的 arxiv_paper_setting.fetch_categories 覆盖）。
# 默认收窄到 cs 大类：避免 13 大类全量抓取触发 arXiv 软节流，且对 cs 方向订阅足够。
DEFAULT_CATEGORIES_TO_FETCH = ["cs"]
# 历史全量清单：需要复现旧 13 大类行为时，设 fetch_categories: all 或显式列出。
LEGACY_ALL_CATEGORIES = [
...
def resolve_categories(default_categories: list[str]) -> list[str]:
    """从 config.yaml 的 arxiv_paper_setting.fetch_categories 解析要抓的分类。

    - 未配置 → 用 default_categories（即 DEFAULT_CATEGORIES_TO_FETCH = ["cs"]）
    - 配置为 "all" → 用 LEGACY_ALL_CATEGORIES（复现旧 13 大类行为）
    - 配置为字符串 → 视为单个分类
    - 配置为列表 → 去重去空白后使用
    """
    config = load_config()
    paper_setting = (config or {}).get("arxiv_paper_setting") or {}
    value = paper_setting.get("fetch_categories")
    if value is None:
        return list(default_categories)
    if isinstance(value, str):
        if value.strip().lower() == "all":
            return list(LEGACY_ALL_CATEGORIES)
        cats = [value.strip()]
...
def fetch_all_domains_metadata_robust(
    ...
    # 2. 遍历分类进行抓取（分类列表由 config.yaml 的 fetch_categories 控制，默认 ["cs"]）
    categories = resolve_categories(DEFAULT_CATEGORIES_TO_FETCH)
    log(f"🏷️  [Global Ingest] 抓取分类：{categories}（共 {len(categories)} 个）")
