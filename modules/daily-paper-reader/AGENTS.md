# AGENTS.md — Daily Paper Reader 项目导读
...
```bash
python -m pytest                 # 全部 Python 测试（pytest.ini 已设 pythonpath=.）
python -m pytest tests/<file>.py # 单个测试文件
python -m pytest tests/<file>.py -k <name>  # 单个用例 / 模糊匹配
node tests/<file>.js             # 前端纯函数测试（无测试框架，直接 node 运行并断言）
```

JS 测试为自包含脚本（`node:assert`，无 package.json），失败时以非零退出码结束；亦可逐一运行全部 `tests/test_*.js`。

**Python 环境（重要，避免用错解释器）**：

本项目本地只认 **两个** Python 环境，其余一律不要用：

| 环境 | 路径（本机） | 用途 |
|------|------|------|
| **`dpr` conda 环境（主环境）** | `C:\Users\86180\miniconda3\envs\dpr\python.exe` | 本地后端、流水线（含 CUDA torch 本地 reranker）、pytest 测试，全部用它 |
| **`.venv-docling`（子进程专用）** | `.venv-docling\Scripts\python.exe` | 仅 Docling 抽图，由 `.env` 的 `DOCLING_PYTHON` 自动调用，无需手动使用 |

- **启动本地后端**：`C:\Users\86180\miniconda3\envs\dpr\python.exe src/local_server.py --port 8567`（或 `bash scripts/run_local.sh`，它经 `scripts/_resolve_python.sh` 自动优先选 dpr 环境）
- **跑测试**：`C:\Users\86180\miniconda3\envs\dpr\python.exe -m pytest`
- **为什么 docling 单独一个环境**：docling 依赖链会强制 `torch==2.13.0`（CPU 版），与 dpr 的 CUDA torch 冲突，故隔离在 `.venv-docling` 子进程里（`scripts/setup_docling.sh` 一键搭建）
- **不要用**：miniconda base、`envs/python_3.8`、项目 `.venv`（已废弃删除）——它们缺 torch/fitz 等关键依赖，流水线会在 Step 3/6 崩溃
- 无 conda 的机器：`scripts/bootstrap_local.sh` 会建项目 `.venv` 兜底（`DPR_INSTALL_MODE=full` 装本地模型依赖）

**依赖清单**（装进 dpr 环境）：

```bash
pip install -r requirements.txt          # 核心
pip install -r requirements-local-models.txt  # 本地 reranker / embedding（torch + transformers）
pip install -r requirements-paper-media.txt  # PDF/图表提取
pip install -r requirements-cpu.txt          # CPU 版（无 GPU，仅供本地调试）
```

**单一可执行入口**：`src/main.py` 顺序编排 Step 0→6，各 Step 依赖 `DPR_RUN_DATE` 环境变量（见 §三）。
...
**召回双模式**（`config.yaml` 的 `local.recall.mode`，或 `.env` 的 `DPR_RECALL_MODE`；默认 `supabase`，面板「召回模式」可切）：
- `supabase`（云端）：上图原链路，Step 1/2.1/2.2 依赖 Supabase RPC。
- `local`（本地）：Step 1L/2L 替代 Step 1/2.1/2.2——arXiv API 按 `submittedDate` 窗口增量幂等 upsert 进本地 SQLite FTS5（`fetch_arxiv_window.py`，多路线抓取：直连→系统代理；同 UTC 日已抓自动跳过，`DPR_LOCAL_FETCH_FORCE=1` 强制），再本地 FTS 词法召回 + embedding 语义精排（`recall_local.py`；embedding 默认经中转站 `/v1/embeddings` 用 qwen3-embedding，失败自动降级本地 bge-small；查询与候选必须合并一次编码）。lane 输出与 2.1/2.2 同构，Step 2.3 起下游完全一致。**本地步骤失败时自动回退云端链路**（arXiv 不可达且本地库为空时），日报不中断；本地库有窗口数据则降级继续。
- 依赖格局（2026-08-29 起）：本地模式下整条链 = arXiv API + 本地 SQLite + 中转站（rerank/LLM/embedding），共享 Supabase 从致命依赖降级为云端模式可选项。实录见 `docs_init/pipeline-timing.md` 运行 C。
...
- **本地召回模式**：`local.recall.mode=local`（或 `DPR_RECALL_MODE`）时改走 Step 1L/2L——arXiv API 窗口增量幂等 upsert 进本地 SQLite FTS5（`local_paper_store.py`）+ 词法/embedding 双 lane（`recall_local.py`；embedding 默认经中转站 `/v1/embeddings`（qwen3-embedding），失败降级本地 bge；查询与候选必须同一次编码）。lane 输出与 2.1/2.2 同构，2.3 起下游复用。arXiv 多路线（直连/系统代理）抓取失败时，本地库已有窗口数据则降级继续；同一 UTC 日已抓取则跳过（`DPR_LOCAL_FETCH_FORCE=1` 强制）
...
- **Reranker 后端四选一**（`RERANK_PROFILE` 预设 / 设置面板「Reranker 精选后端」，面板选择保存即生效并清除 .env 残留覆盖键）：
  - `public-sinksilk-rerank`：自建 new-api 中转（模型名 `qwen3-reranker`；渠道不认 `top_n` 参数，客户端已改为不传并自行截断）
  - `public-zwwen-rerank`：zwwen.online 公益端点（免配置，内置公共密钥兜底）
  - `siliconflow-qwen3-0.6b`：SiliconFlow API（`src/reranker_api.py`）
  - `local-qwen3-0.6b`：本地 `Qwen3-Reranker-0.6B`（yes/no token logits → P(yes) 为分数）
  - 坑：非空 `RERANK_PROVIDER`/`RERANK_MODEL`/`RERANK_API_BASE_URL`/`PUBLIC_RERANK_API_BASE_URL`/`SILICONFLOW_RERANK_URL` 会覆盖 profile 自带值；选定 profile 时这些键必须清空（面板注入会自动清）
...
### 11. 综述生成链路 `src/survey_pipeline.py` + `src/survey_docs.py`

- **本地后端原生功能**：`#/survey` 页 → `POST /api/survey` 异步 Job（connect.job.v1，`SurveyJobStore`），不再依赖外部网关/子项目
- **十阶段编排**（结构平移自已移除的 paper_agent 多智能体设计）：种子分析（种子论文锚定任务范式/派生查询/引文直取，可选）→ 四路召回融合（本地 BM25+向量 / 种子引文 / **Kaggle 快照 FTS 粗筛（默认主路）** / DeepXiv 语义（默认关——外部 token 限额/波动，前端勾选补充被引数与最新论文），RRF 去版本号去重）→ **本地语义粗排**（万级候选→300，bge 本地直连）→ Reranker 精选（3.rank_papers，失败回退 RRF 序）→ 逐篇结构化抽取（relevance<4 丢弃 + 范式门）→ 聚类（肘部法则 KMeans + LLM 簇命名）→ 核心论文 PDF 深读（`archive/survey_texts/` 缓存）→ 簇深析 + 全局分析（注入候选池年份/类别画像补历史脉络）→ 导演大纲（每节标注覆盖簇）→ 分节并行写作（**并发闸门=2，防 429**，引用 [n] 装配时校验）→ 审校
- **Kaggle 快照粗筛路** `src/kaggle_arxiv.py`：Cornell-University/arxiv 官方快照（~250 万篇，~4GB JSON）本地 SQLite FTS5 词法粗筛（零网络无限流，`_query_to_fts` AND/OR 双查询），认证 `KAGGLE_API_TOKEN`（KGAT Bearer）/`KAGGLE_USERNAME`+`KAGGLE_KEY`（Basic），GCS 下载需 `KAGGLE_TRUST_ENV=1` 走代理；构建 `scripts/build_kaggle_arxiv_index.py --download`（建议每周刷新；快照周级滞后由 DeepXiv 补位，无被引数）
- **lane 对比统计（A/B 核心）**：每路召回记 latency/hits，终稿按 `recall_sources` 聚合 papers_in_final/avg_relevance/avg_paradigm_consistency + 漏斗 `{fts_candidates→embed_pool→rerank_in→final}` + 种子引文覆盖率，全部落 `report_meta`（`lane_stats`/`funnel`/`seed_citation_coverage`）
- **报告落盘**：`docs/survey/<slug>-<hash>.md`（255 字节短名约定），`update_sidebar_with_survey` 以 `<!--dpr-survey:<report_id>-->` 标记幂等注册 `* Survey Reports` 分组
- **前端**：`app/survey-generator.js`（同源 `/api/survey` 轮询 + 事件去重 + 阶段权重进度条 + DeepXiv/Kaggle 独立开关 + 粗筛规模三档 3千/1万/3万）；`dpr-sidebar.js` 解析 surveys 分组（默认收起，搜索/未读筛选模式不索引）
- **取消与预算**：协作式取消（阶段边界检查）+ 40 分钟墙钟兜底；模型可用 `DPR_SURVEY_MODEL` 覆盖
...
- `src/local_server.py`：本地化部署后端，无需 GitHub Actions；启动：`python src/local_server.py --serve`，默认 `http://127.0.0.1:8567`。AI 问答通过 `/api/chat` 代理转发，密钥存本机 `config.yaml`（`local.chat`）或 `.env`；定时流水线由 `local.schedule` 控制。
...
- **Python 测试**：`pytest`（`pytest.ini`），`tests` 目录下含 60+ 测试文件，覆盖各服务模块
- **JS 测试**：使用 Vitest 运行 `*.test.js` 文件
...
### TODO
1. ~~总结上传pdf解读，没有论文标题作者信息~~ ✅ 已完成（`local_server.py` 总结 Job：上传 PDF 先按文件名 arXiv id 反查官方元数据；否则 LLM 从正文开头抽 title/authors/venue/source_kind，失败回退旧行为；URL 分支同样补作者/来源）
2. ~~Source: arxiv ；论文来源~~ ✅ 已完成（`_resolve_summarize_source` 给真实 source token（arxiv/biorxiv/medrxiv/chemrxiv/conference/pdf/web），venue 单独写 front matter 渲染 venue chips；`6.generate_docs.py` 图表门槛放行带 pdf_bytes 的未知来源）
#### 3. 日报时间范围限制？？？做了吗？
4. Reranker 具体速度对比，是否收费限制额度？
