...
class AuthorIntelSkill:
    ...
    async def _run_inner(self, ctx: SkillContext, **kwargs) -> SkillResult:
        ...
        searcher = AuthorSearcher(
            ...
            light_api_key=getattr(config, 'light_api_key', ''),
            light_base_url=getattr(config, 'light_base_url', ''),
            search_backend=getattr(config, 'search_backend', 'zhipu_native'),
            scraper_api_keys=getattr(config, 'scraper_api_keys', []),
        )
