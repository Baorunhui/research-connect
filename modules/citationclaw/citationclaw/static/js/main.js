...
function initIndexPage() {
    ...
    (async () => {
        try {
            ...
            if (el('idx-openai-model')) {
                ...
                // If saved model doesn't match any search option, leave blank
                // (adding options to the <select> in index.html is the proper fix;
                //  silently forcing a Gemini default would overwrite user config)
            }
    ...
    async function saveIndexConfig() {
        ...
        try {
            ...
            if (!body.profile_fallback_api_keys?.length && existing.profile_fallback_api_keys?.length) delete body.profile_fallback_api_keys;
    ...
    window.pretestSearchLLM = function() {
        ...
        _runPretest('/api/pretest/search_llm', {
            api_key: key,
            base_url: baseUrl,
            model: model || cfg.openai_model || 'gemini-3-flash-preview-search',
