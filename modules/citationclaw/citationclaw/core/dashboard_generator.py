...
class DashboardGenerator:
    ...
    def _compute_stats(self, papers, total_papers, top_scholars, all_scholars):
        ...
        def _valid_country(c):
            ...
            if s.lower() in ('', 'nan', 'none', 'null', '-'):
                return False
            # "国际" is a placeholder for AAAI/IEEE Fellows whose country is
            # unknown — exclude it from country distributions & counts.
            if s == '国际':
                return False
...
<div class="download-bar">
  ...
  {dl_links}
        ...
        # ── Country distribution cards: only render charts that have data ──
        _country_cards = []
        _country_js = []
        if cp_sorted:
            _country_cards.append(
                '<div class="card"><div class="card-title"><div class="card-title-dot sage"></div>'
                '第一作者国家/地区分布（全部施引文献）</div>'
                '<div style="position:relative;height:200px"><canvas id="cCountryAll"></canvas></div></div>')
            _country_js.append(f"makeCountryChart('cCountryAll', {country_p_labels}, {country_p_data});")
        if cr_sorted:
            _country_cards.append(
                '<div class="card"><div class="card-title"><div class="card-title-dot"></div>'
                '知名学者国家/地区分布</div>'
                '<div style="position:relative;height:200px"><canvas id="cCountryRenowned"></canvas></div></div>')
            _country_js.append(f"makeCountryChart('cCountryRenowned', {country_r_labels}, {country_r_data});")
        if ct_sorted:
            _country_cards.append(
                '<div class="card"><div class="card-title"><div class="card-title-dot amber"></div>'
                '顶尖学者国家/地区分布</div>'
                '<div style="position:relative;height:200px"><canvas id="cCountryTop"></canvas></div></div>')
            _country_js.append(f"makeCountryChart('cCountryTop', {country_t_labels}, {country_t_data});")
        _n_country = len(_country_cards)
        if _n_country >= 3:
            country_grid_html = f'<div class="grid-3">{"".join(_country_cards)}</div>'
        elif _n_country == 2:
            country_grid_html = f'<div class="grid-2">{"".join(_country_cards)}</div>'
        elif _n_country == 1:
            country_grid_html = f'<div class="grid-1">{"".join(_country_cards)}</div>'
        else:
            country_grid_html = ''
        country_js_calls = "\n".join(_country_js)
