...
class ScholarSearchAgent:
    ...
    def __init__(self, api_key: str = "", base_url: str = "", model: str = "",
                 api_keys=None, log_callback=None):
        """api_keys: optional pool of keys (round-robin, 429 auto-skip).
        Falls back to the single api_key when the pool is empty."""
        pool = [k.strip() for k in (api_keys or []) if k and k.strip()]
        if api_key and api_key.strip() and api_key.strip() not in pool:
            pool.append(api_key.strip())
        self._api_keys = pool
        self._key_index = 0
        self._exhausted: set = set()  # key indices skipped after 429
    ...
    def _pick_key(self) -> Optional[int]:
        """Round-robin pick of the next non-exhausted key index (None if all dead)."""
        n = len(self._api_keys)
        if n == 0:
            return None
        for off in range(n):
            i = (self._key_index + off) % n
            if i not in self._exhausted:
                self._key_index = (i + 1) % n
                return i
        return None

    def _ensure_client(self, key: str):
    ...
    async def search_paper_authors(self, paper_title: str, authors: List[dict]) -> List[ScholarResult]:
        ...
        while True:
            ...
            if key_idx is None:
                self._log("    ⚠ 搜索LLM所有 key 均不可用（429/额度耗尽），跳过学者搜索")
                return []
            ...
            try:
                ...
                if e.status_code in (401, 403, 429) and len(self._api_keys) > 1:
                    # 余额不足/限流/无效 key → 换下一个 key 重试
                    self._exhausted.add(key_idx)
                    self._log(
                        f"    ⚠ key #{key_idx + 1} (…{key[-4:]}) {e.status_code} 不可用，切换下一个 key"
                    )
                    continue
