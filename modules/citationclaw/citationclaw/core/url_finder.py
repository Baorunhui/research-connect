...
class PaperURLFinder:
    ...
    async def _s2_fallback(self, paper_title: str) -> Optional[str]:
        """Fallback to Semantic Scholar when ScraperAPI fails.

        Searches S2 by title and returns a sentinel URL of the form
        ...
        try:
            ...
            self.log(f"[URL查找] S2 兜底异常: {e}")
            return None
        if not paper or not paper.get("s2_id"):
            self.log(f"[URL查找] S2 未找到该论文")
            return None
        s2_id = paper["s2_id"]
        self.log(f"[URL查找] S2 找到论文 (s2_id={s2_id})")
        sentinel = f"s2:{s2_id}"
        self._persist_mapping(paper_title, sentinel)
        return sentinel
