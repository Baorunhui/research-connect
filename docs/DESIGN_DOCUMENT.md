...
| 公网协作层 | Report Hub | 邀请注册、安装隔离、固定站点、通用命令队列、进度和报告托管 |
...
- 通过限次、限时邀请码为每个本机安装签发独立 agent token，服务端只保存哈希；
- 为 Daily Paper 和 CitationClaw 分配稳定站点 URL；
- 托管本机 Connect Hub 上传的配置页面，但不保存或解释 Provider 配置；
- 保存稳定站点、运行快照和客户端上传的静态资源；
- 支持大站点分片上传和原子替换；
- 提供受限动态命令队列，使公网原版网页可以操作本机后端；
- 在用户电脑关机后继续提供已经发布的历史网页。

安装 token 只能访问本安装拥有的站点、文件和命令。公网 HTTP API 不保留管理员绕过凭证；服务器管理员通过容器内 CLI 创建邀请、轮换/撤销安装并按 `install_id` 清理数据。
